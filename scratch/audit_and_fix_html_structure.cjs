...
// Standard Banner & Header & Drawer Template
function buildHeader(activePage) {
  ...
  const desktopListItems = links.map(l => {
    const isActive = l.label === activePage;
    const cls = isActive
  ...
  const mobileListItems = links.map(l => {
    const isActive = l.label === activePage;
    const cls = isActive
      ? 'px-3 py-2.5 rounded-lg bg-white/15 text-[#d4af37]'
      : 'px-3 py-2.5 rounded-lg hover:bg-white/10 text-white/90';
    return `        <a href="${l.href}" class="${cls}">${l.label}</a>`;
  }).join('\n') + `\n        <a href="contact.html" class="${activePage === 'Contact' ? 'px-3 py-2.5 rounded-lg bg-white/15 text-[#d4af37]' : 'px-3 py-2.5 rounded-lg hover:bg-white/10 text-white/90'}">Contact</a>`;
        ...
        <div class="flex items-center gap-3 shrink-0">
          <a href="index.html" class="flex items-center gap-2 sm:gap-3 group focus-visible:outline-none shrink-0" aria-label="DYUTI 2027 — Return to Homepage">
            <div class="h-10 w-10 sm:h-12 sm:w-12 lg:h-13 lg:w-13 rounded-full bg-white p-1 shadow-md flex items-center justify-center shrink-0 transition-transform duration-200 group-hover:scale-105 border border-slate-200">
              <img src="https://dyuti.in/assets/images/dyutilogoog.jpg" alt="DYUTI Emblem" class="h-full w-full object-contain rounded-full">
            </div>
      ...
      <div class="flex items-center justify-between pb-4 border-b border-white/15">
        <div class="flex items-center gap-3">
          <div class="h-10 w-10 rounded-full bg-white p-1 shadow-md">
            <img src="https://dyuti.in/assets/images/dyutilogoog.jpg" alt="DYUTI Logo" class="h-full w-full object-contain rounded-full">
          </div>
          <div>
...
allFiles.forEach(file => {
  ...
  const bodyCloseTagIndex = content.indexOf('>', bodyIndex);
  const headAndBodyOpen = content.slice(0, bodyCloseTagIndex + 1);
  ...
  if (mainStart !== -1 && mainEnd !== -1) {
    ...
    const mainOpenEnd = content.indexOf('>', mainStart);
    mainContent = content.slice(mainOpenEnd + 1, mainEnd).trim();
  } else {
    // Extract content between header/aside and footer
    const asideEnd = content.indexOf('</aside>');
    const drawerEnd = content.indexOf('</div>\n  </aside>');
    let splitPoint = asideEnd !== -1 ? asideEnd + 8 : -1;
    if (splitPoint === -1) {
      const headerEnd = content.indexOf('</header>');
      splitPoint = headerEnd !== -1 ? headerEnd + 9 : -1;
    }
    if (splitPoint !== -1) {
      mainContent = content.slice(splitPoint, footerIndex).trim();
    } else {
      console.warn('Could not determine main content for', file);
      return;
    }
  }

  const footerAndScripts = content.slice(footerIndex);

  const activePage = pageTitles[file] || '';
  const headerHtml = buildHeader(activePage);

  const finalHtml = `${headAndBodyOpen}
...
${footerAndScripts}`;
  ...
  console.log('Processed & Standardized:', file);
});
