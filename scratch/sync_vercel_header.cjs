...
function generateHeader(activeKey) {
  ...
  const desktopLiItems = links.map(link => {
    ...
    const activeClasses = isActive 
      ? 'font-bold text-[#d4af37] border-b-2 border-[#d4af37] pb-0.5' 
      : 'font-semibold text-white/90 hover:text-[#d4af37] transition-all';
    return `            <li><a href="${link.href}" class="relative px-2.5 lg:px-3 2xl:px-3.5 py-1.5 text-[13px] 2xl:text-[14.5px] font-sans whitespace-nowrap ${activeClasses}">${link.label}</a></li>`;
  }).join('\n');
...
pages.forEach(({ file, key }) => {
  ...
  console.log(`Updated complete header with attached announcement in ${file}`);
});
